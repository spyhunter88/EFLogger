...
namespace FrameLog.Logging
{
        ...
        where TChangeSet : IChangeSet<TPrincipal>
    {
            ...
            foreach (string propertyName in getChangedProperties(entry))
            {
                if (filter.ShouldLog(entry.Entity.GetType(), propertyName))
                {
                    ...
                    foreach (var valuePair in valuePairs)
                    {
                        ...
                        {
                            // Specified to the reference (almost ID) while Deleted is marked
                            // to avoid Null ObjectReference.
                            string reference = DataContextHelper.GetReferenceForObject(context, entity);
                        ...
                        else
                        {
                            var pair = valuePair;
                            recorder.Record(entry.Entity,
                                () => DataContextHelper.GetReferenceForObject(context, entity),
                                valuePair.PropertyName,
                                pair.NewValue);
                        }
                    }
        ...
        private string getKeyReference(EntityKey key)
        {
            ...
            return DataContextHelper.GetReferenceForObject(context, entity);
        }
